...
AssetFlow Invest v0.13.1 起改為登入後自動載入雲端庫存儀表板，並仍須先使用 Google 帳號登入才會顯示主畫面。App 會將台股與美股庫存分開呈現，第一份快照視為初始庫存，後續快照會與前一份快照比較，計算每日布局股數增減與估算布局成本。方舟建議水位百分比會優先讀取 `水位` tab 的台股/美股市場層級水位。截圖解析時會先估算前方完整圓圈數，和實際解析出的庫存筆數互相比對，協助發現漏抓列。
