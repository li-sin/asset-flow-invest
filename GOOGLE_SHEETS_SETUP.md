...
AssetFlow Invest v0.9.4 起改為必須先使用 Google 帳號登入，才會顯示主畫面與讀取雲端庫存。`2026 Invest` 不需要開啟「知道連結的人可檢視」；建議只分享給自己的 Google 帳號。

## 需要準備

- Spreadsheet ID：`1adzBH3WaQ_pUgXeSKb2AeGkQE5pXejhHBxQ6MV8XtSI`
- Google OAuth Client ID：App 已內建預設值 $clientId，需要更換時可在 App 內設定。
- 允許登入的 Google Email：填自己的 Google 帳號。
...
5. 填入允許登入的 Google Email。
6. 按 `使用 Google 登入`，並選擇同一個 Google 帳號。
