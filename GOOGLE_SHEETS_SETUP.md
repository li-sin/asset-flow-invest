...
AssetFlow Invest v0.11.3 起改為登入後自動載入雲端庫存儀表板，並仍須先使用 Google 帳號登入才會顯示主畫面。App 會避免重複寫入同日期、同市場、同內容的快照，並可清理已存在的重複雲端快照；方舟建議水位百分比會優先讀取 `水位` tab，手動修改時才存於瀏覽器本機設定。`2026 Invest` 不需要開啟「知道連結的人可檢視」；建議只分享給自己的 Google 帳號。
