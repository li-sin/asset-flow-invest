...
AssetFlow Invest v0.12.0 起改為登入後自動載入雲端庫存儀表板，並仍須先使用 Google 帳號登入才會顯示主畫面。App 會將台股與美股庫存分開呈現，方舟建議水位百分比會優先讀取 `水位` tab 的台股/美股市場層級水位，手動修改時才存於瀏覽器本機設定。新增截圖會開啟獨立輸入畫面，不再常駐首頁。
