...
`QUOTE_PROXY_APPS_SCRIPT.gs` is the Google Apps Script source for the Yahoo Finance proxy. The deployed proxy must support `mode=history` for historical P/L trend charts; otherwise those charts intentionally show that historical close data is unavailable instead of silently using today's price for old snapshots.
