...
v0.11.3 keeps Google sign-in before the app shell is shown, then automatically loads the latest cloud inventory into a dashboard-first home screen. Screenshot cards are now treated as the snapshot vault, while the dashboard shows current holdings, estimated cost water levels, Ark suggested target levels from the `水位` sheet tab, recent snapshot trends, daily layout totals, and position details. The app blocks duplicate same-day snapshots and can clean existing duplicate cloud snapshots while keeping the newest copy. Cloud inventory reads and writes use the Google Sheets API with OAuth, so the `2026 Invest` sheet can stay private and shared only with the owner's Google account.
